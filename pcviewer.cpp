...
{
    DenoisingModule module1;
    PointCloud<PointXYZRGBA>::Ptr cloud_filtered_;
    cloud_filtered_.reset(new PointCloud<PointXYZRGBA>);

    module1.SetInputCloud(cloud_);
    module1.pass_through_filter(cloud_filtered_);
    viewer_->updatePointCloud(cloud_filtered_,"cloud");
    viewer_->resetCamera();
    ui->widget->update();
}
